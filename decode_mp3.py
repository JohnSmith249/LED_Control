...
def midi_to_midievents(stream):
    ...
    for ev in m.tracks[0].events:
        if ev.type == "DeltaTime":
            ...
            yield ("noteon", ev.track.index, ev.pitch, ev.velocity)
        elif ev.type == "NOTE_OFF":
            yield ("noteoff", ev.track.index, ev.pitch)
        elif ev.type == "SET_TEMPO":
            pass  # TODO (?) ...
        else:
            print
            "midi warning: event", ev, "ignored"
...
def wav_to_rawpcm(stream):
    ...
    if stream.read(4) != "fmt ":
        raise (Exception, "fmt section expected")
    if unpack("<L", stream.read(4))[0] != 16:
        raise (Exception, "fmt section should be of size 16")
    wFormatTag = unpack("<H", stream.read(2))[0]
    if wFormatTag != 1:
        raise (Exception, "PCM format expected but got " + hex(wFormatTag))
    ...
    if dwSamplesPerSec != sampleRate:
        raise (Exception, "we expected " + str(sampleRate) + " Hz but got " + str(dwSamplesPerSec))
    if wBitsPerSample != 16:
        raise (Exception, "we expected 16 bits per sample but got " + str(wBitsPerSample))
    if wBlockAlign != 2:
        raise (Exception, "we expected block align 2 but got " + str(wBlockAlign))
    if wChannels != 1:
        raise (Exception, "we expected 1 channel but got " + str(wChannels)

    if stream.read(4) != "data" :
        raise (Exception, "data section expected")
